...
class Network(object):
    ...
    def Bilstm(self, input, d_i, d_h, d_o, name, trainable=True):
        ...
        with tf.variable_scope(name) as scope:
            ...
            img = tf.reshape(img, [N * H, W, C])  # batch, timestep, embedding_size

            # Debug
            img_shape = tf.shape(img)
            tf.add_to_collection('bilstm_image_shape', img_shape)
            # --Debug

            img.set_shape([None, None, d_i])
            # Debug
            after_set_shape = tf.shape(img)
            tf.add_to_collection('bilstm_imageset_shape', after_set_shape)
            # --Debug
        ...
        def regularizer(tensor):
            with tf.name_scope(scope, default_name='l2_regularizer', values=[tensor]):
                l2_weight = tf.convert_to_tensor(weight_decay,
                                                 ...
                                                 name='weight_decay')
                # return tf.mul(l2_weight, tf.nnocr.l2_loss(tensor), name='value')
    ...
    def build_loss(self, ohem=False):
        # classification loss
        rpn_cls_score = self.get_output('rpn_cls_score_reshape')   # shape (?, ?, ?, 2)
        print('build_loss, rpn_cls_score shape:', rpn_cls_score.shape)

        rpn_cls_score = tf.reshape(rpn_cls_score, [-1, 2]) # shape (HxWxA, 2)
        rpn_label = tf.reshape(self.get_output('rpn-data')[0], [-1])  # shape (HxWxA)

        # Debug
        tf.add_to_collection('rpn_cls_score_value', rpn_cls_score)
        # Debug
        ...
        # Debug
        tf.add_to_collection('rpn_cross_entropy_n_value', rpn_cross_entropy_n)
        cross_shape = tf.shape(rpn_cross_entropy_n)
        # Debug

        # box loss
        rpn_bbox_pred = self.get_output('rpn_bbox_pred')  # shape (1, H, W, Ax4)
        print('build_loss, rpn_bbox_pred shape:', rpn_bbox_pred.shape)
